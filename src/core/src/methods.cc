...
inline std::vector<double> utils::StepRK_SOE(std::function<double(double,double,double)> rhs1,std::function<double(double,double,double)> rhs2, const double& x, const double& u, const double& y, const double& step) {
    //LOG_DEBUG_CLI("Start StepRK4_SOE step with following config", x, u, step);
...
resultTable utils::RK_SOE(std::function<double(double, double, double)> rhs1, std::function<double(double, double, double)> rhs2, const config& cfg) {
    //LOG_INFO_CLI("Start RK4_SOE with following config", cfg);
    resultTable table;
    double xi, x_min, x_max, ui, yi, stepi, N_max, eps;
    uint C1, C2;
    ...
    if (cfg.LEC) {
        ...
        while (xi >= x_min && xi+stepi < x_max && i < N_max && yi>cfg.A && ui>cfg.A) {
            //LOG_DEBUG_CLI("Start RK4_SOE with localstecpcontrol", cfg);
            tmp1 = StepRK_SOE(rhs1, rhs2, xi, ui, yi, stepi);
            tmp2 = StepRK_SOE(rhs1, rhs2, xi, ui, yi, stepi/2);
            tmp2 = StepRK_SOE(rhs1, rhs2, xi+stepi/2, tmp2.at(0), tmp2.at(1), stepi/2);
            Old_LocalError = LocalError;
